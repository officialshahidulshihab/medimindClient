...
export function useSymptomChecker() {
  ...
  const sendMessage = useCallback(
    async (userInput: string) => {
      ...
      try {
        ...
        // Send to AI
        const aiResponse = await sendSymptomMessage(
  ...
  async function tryParseReport(aiText: string) {
    ...
    const urgencyMatch =
      aiText.match(/Urgency[:\s*]+(\d+)\s*\/\s*10/i) ||
      aiText.match(/urgency[:\s]+(\d+)/i);
    if (urgencyMatch) {
  ...
  const loadSession = useCallback((loadedSession: SymptomSession) => {
    ...
    if (loadedSession.turns && loadedSession.turns.length > 0) {
      ...
      setMessages([
        {
          role: "user",
          content: loadedSession.initialSymptoms?.[0] || "Unknown",
          timestamp: new Date(loadedSession.createdAt),
        },
      ]);
    }
        ...
        urgencyColor:
          score >= 8
            ...
            : score >= 5
              ...
              : "var(--success)",
        recommendedSpecialty:
          loadedSession.recommendedSpecialty || "General Physician",
      });
    } else {
      setReport({});
    }
