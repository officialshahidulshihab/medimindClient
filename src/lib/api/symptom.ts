...
export async function createSymptomSession(
  initialSymptoms: string[],
): Promise<SymptomSession> {
  const { data } = await api.post("/symptom-sessions", { initialSymptoms });
  return data.data;
}
...
export async function sendSymptomMessage(
  ...
  const { data } = await api.post("/ai/symptom-chat", {
    sessionId,
    userMessage,
  });
  return data.data;
...
export async function getSessionById(id: string): Promise<SymptomSession> {
  const { data } = await api.get(`/symptom-sessions/${id}`);
  return data.data;
}

// Get all sessions for history
export async function getSessions(): Promise<SymptomSession[]> {
  const { data } = await api.get("/symptom-sessions");
  return data.data;
}
...
export async function completeSession(
  ...
  const { data } = await api.patch(`/symptom-sessions/${id}/complete`, payload);
  return data.data;
}
